    ...
    return (
        <div className="flex flex-col gap-6 w-full">
            ...
            {items.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {items.map((item, index) => {
                        ...
                        return (
                            <div key={item.id}
                                 className={`relative group rounded-xl overflow-hidden shadow-sm aspect-[3/4] flex flex-col transition-all duration-200 ${isFirst ? 'border-black dark:border-white border-[3px]' : 'border border-gray-200 dark:border-zinc-800'}`}>
                                {isFirst && (
                                    <div
                                        className="absolute top-2 left-2 z-10 bg-black dark:bg-white text-white dark:text-black text-[10px] font-bold px-2 py-1 rounded-full shadow-sm uppercase tracking-wider">
                                        Головне
                                    </div>
                                )}

                                <Image src={imgSrc} alt="Wine preview" fill sizes="(max-width: 768px) 50vw, 25vw"
                                       className="object-cover z-0"/>

                                {/* Overlay controls */}
                                <div
                                    className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity z-10">
            ...
            <div className="flex flex-col gap-1.5 flex-1 w-full relative mt-2">
                <label htmlFor="file-upload" className="text-sm font-medium text-gray-900 dark:text-white">Вибрати фото
                    (можна декілька)</label>
                <input
                    type="file"
                    id="file-upload"
                    accept="image/*"
                    ...
                    className="h-12 w-full rounded-lg border border-gray-300 dark:border-zinc-700 bg-transparent text-sm transition-colors focus:border-black dark:focus:border-white focus:outline-none file:mr-4 file:h-full file:px-4 file:border-0 file:bg-gray-100 dark:file:bg-zinc-800 file:text-gray-900 dark:file:text-white file:font-medium hover:file:bg-gray-200 dark:hover:file:bg-zinc-700 cursor-pointer"
                />
            </div>
        </div>
    )
